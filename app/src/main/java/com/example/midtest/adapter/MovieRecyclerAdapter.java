...
import com.example.midtest.helper.MoviesRecyclerViewHolder;
import com.example.midtest.R;
import com.example.midtest.model.Movies;
import com.example.midtest.view.Detail;
    ...
    public void onBindViewHolder(@NonNull final MoviesRecyclerViewHolder holder, int position) {
        ...
        if (holder.getItemViewType() == 1){
            ...
            holder.cardView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Toast.makeText(context, "Tv Show : "+holder.title.getText().toString(), Toast.LENGTH_SHORT).show();
                }
            });
            holder.title.setText(list.get(position).getTitle());
        }
